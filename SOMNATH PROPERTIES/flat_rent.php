...
<body>
    ...
    if(ISSET($_POST['search'])){ // ISSET checks whether the button is clicked
        ...
        $sel = "SELECT * FROM flat_rent WHERE DETAILS LIKE '%$keyword%'"; //simply storing query as a string
        $que = mysqli_query($con,$sel);// all the records stored by the query
    }
      else{
        $sel = "SELECT * FROM flat_rent"; //simply storing query as a string
        $que = mysqli_query($con,$sel);// all the records stored by the query
      }
