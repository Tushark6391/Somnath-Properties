...
<head>
    ...
    <title>Shops for Rent</title>
</head>
<body>
    <h1>Shops for Rent</h1>
   ...
   if(ISSET($_POST['search'])){ // ISSET checks whether the button is clicked
    ...
    $sel = "SELECT * FROM shop_rent WHERE DETAILS LIKE '%$keyword%'"; //simply storing query as a string
    $que = mysqli_query($con,$sel);// all the records stored by the query
}
  else{
    $sel = "SELECT * FROM shop_rent "; //simply storing query as a string
    $que = mysqli_query($con,$sel);// all the records stored by the query
  }
